...
BEGIN
	...
	BEGIN
		...
		(
			...
			ORDER BY 
				(SELECT COUNT(1) FROM stg_permit p WHERE p.stg_id = @stg_id AND p.league_id = @league_id AND p.home_team_id = m.home_team_id AND NOT EXISTS (SELECT permit_id FROM stg_matchup WHERE stg_id = @stg_id AND league_id = @league_id AND permit_id = p.permit_id) AND NOT EXISTS (SELECT permit_id FROM stg_permit WHERE stg_id = @stg_id and league_id = @league_id AND home_team_id = m.away_team_id AND MONTH(p.permit_date) = MONTH(permit_date) AND DAY(p.permit_date) = DAY(permit_date)) AND NOT EXISTS (SELECT mt.matchup_id FROM stg_matchup mt INNER JOIN stg_permit pt ON mt.stg_id = pt.stg_id AND mt.league_id = pt.league_id AND mt.permit_id = pt.permit_id WHERE mt.stg_id = @stg_id AND mt.league_id = @league_id AND mt.away_team_id = m.away_team_id AND MONTH(pt.permit_date) = MONTH(p.permit_date) AND DAY(pt.permit_date) = DAY(p.permit_date)))
				,newid()
		)
